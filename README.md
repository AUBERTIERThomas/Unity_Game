...
1. L'exécutable (et donc le jeu) a un ptit logo rouge, tu devrais pas le rater. Tu peux le lancer tel quel et tout devrait 
marcher (j'espère vraiment ptn !).
2. Le fichier de sauvegarde s'appelle `_SaveTime`. Il va stocker le meilleur temps réalisé sur chaque niveau terminé, en plus 
...
4. Les fichiers commençant par `_TimeToBeat`[...] contiennent des temps à battre pour chaque niveaux (avec trois difficultés).
...
1. Depuis combien de temps tu bosses dessus ?

    Depuis Février 2024 (pour le projet étudiant). Je suis pas très rapide, vu que je me presse pas trop et suis pas du tout à temps plein. 
    Mais on avance !

2. Vraiment la manette c'est mieux que le clavier ?
...
4. J'ai beau tryhard, j'arrive pas à avoir un temps or. Sont-ils vraiment faisables ?
...
5. Comment tu fais tes graphismes ?
